...
export default function UploadPage() {
  ...
  const status = useQuery({
    ...
    queryFn: () => apiGet("/uploads/status"),
    refetchInterval: 10_000,
  });

  const latest = useQuery({
    queryKey: ["upload-latest"],
    queryFn: () => apiGet("/uploads/latest"),
  });
  ...
  const upload = useMutation({
    mutationFn: async (selected) => {
      ...
      return apiPostForm("/uploads/workbook", form);
    },
...
function MappingStatus({ status, loading, error }) {
...
function MappingTab({ mapped, status }) {
