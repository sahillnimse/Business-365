...
const tabSchema = z.object({ tab: z.enum(["connections", "azure", "bc365"]).optional() });

export const Route = createFileRoute("/settings")({
  validateSearch: tabSchema,
  head: () => ({
    meta: [
      { title: "Settings — PO‑Validator" },
      { name: "description", content: "Manage data source connections and authentication." },
    ],
  }),
...
function ConnectionsTab() {
  const q = useQuery({ queryKey: ["health-full"], queryFn: () => apiGet<HealthResp>("/health") });
